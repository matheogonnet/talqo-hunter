...
import { applyProspectSearchFilters, matchesProspect } from '@/lib/search'
...
export default async function DashboardPage({ searchParams }: PageProps) {
  ...
  if (!q?.trim()) {
    query = applyProspectSearchFilters(query, { status, sector })
  } else {
  ...
  let prospects = (data ?? []) as ProspectWithDecisionMakers[]
  if (q?.trim()) {
    prospects = prospects.filter((p) => matchesProspect(p, q.trim()))
  }
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              <span className="md:hidden">
                {prospects.length} prospect{prospects.length > 1 ? 's' : ''} actif{prospects.length > 1 ? 's' : ''} — fais défiler les colonnes puis maintiens la poignée pour déplacer une carte.
              </span>
              <span className="hidden md:inline">
                {prospects.length} prospect{prospects.length > 1 ? 's' : ''} actif{prospects.length > 1 ? 's' : ''} — glisse les cartes pour changer d&apos;étape.
              </span>
            </p>
          </div>
          <AddProspectDialog />
        </div>
        <ProspectFilters counts={{ total: prospects.length, byStatus }} />
      </div>
